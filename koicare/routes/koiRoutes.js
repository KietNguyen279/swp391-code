...
router.post("/", verifyTokens, verifyMemberAndShopAndAdminRole, (req, res) => {
  ...
  if (typeof image !== "string" || image.length === 0) {
    return res.status(400).json({
      message: "Invalid image. Image URL must be a non-empty string.",
    });
  }
  if (typeof body_shape !== "string" || body_shape.length === 0) {
    return res.status(400).json({
      message: "Invalid body_shape. Body shape must be a non-empty string.",
    });
  }
  if (isNaN(age) || age < 0) {
    return res
      .status(400)
      .json({ message: "Invalid age. Age must be a non-negative number." });
  }
  if (isNaN(size) || size < 0) {
    return res
      .status(400)
      .json({ message: "Invalid size. Size must be a non-negative number." });
  }
  if (isNaN(weight) || weight < 0) {
    return res.status(400).json({
      message: "Invalid weight. Weight must be a non-negative number.",
    });
  }
...
router.put(
  ...
  (req, res) => {
    ...
    if (
      body_shape &&
      (typeof body_shape !== "string" || body_shape.length === 0)
    ) {
      return res.status(400).json({
        message: "Invalid body_shape. Body shape must be a non-empty string.",
      });
  ...
  (req, res) => {
    ...
    Koi.deleteKoiById(koiId, (error, result) => {
      if (error) {
        ...
        res.json({
          message: "Koi and related growth records deleted successfully",
        });
      } else {
        res.status(404).json({ message: "Koi not found" });
      }
