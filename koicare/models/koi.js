...
const createKoi = (
  ...
  if (typeof image !== "string" || image.length === 0) {
    return callback(
      new Error("Invalid image. Image URL must be a non-empty string."),
      null
    );
  }
  if (typeof body_shape !== "string" || body_shape.length === 0) {
    return callback(
      new Error("Invalid body_shape. Body shape must be a non-empty string."),
      null
    );
  }
  if (typeof age !== "number" || age <= 0) {
    return callback(
      new Error("Invalid age. Age must be a number greater than 0."),
      null
    );
  }
  if (typeof size !== "number" || size <= 0) {
    return callback(
      new Error("Invalid size. Size must be a number greater than 0."),
      null
    );
  }
  if (typeof weight !== "number" || weight <= 0) {
    return callback(
      new Error("Invalid weight. Weight must be a number greater than 0."),
      null
    );
  }
